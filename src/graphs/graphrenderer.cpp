...
: graph(graph), cellW(cellW), cellH(cellH), pathStyle(LINES) {
}
...
void GraphRenderer::drawPath(RenderWindow &window, const Vector2f &topLeft,
    ...
    switch (pathStyle) {
        case LINES:
            drawLinePath(window, topLeft, path);
            break;
        case DOTS:
            drawDottedPath(window, topLeft, path);
            break;
    }
}

void GraphRenderer::drawLinePath(sf::RenderWindow &window, const sf::Vector2f &topLeft,
...
void GraphRenderer::drawDottedPath(sf::RenderWindow &window, const sf::Vector2f &topLeft,
                                   const std::vector<std::pair<int, int>> &path) const {
    float lineThickness = 4.0f;
...
//        c.setFillColor(Color::Magenta);
//        c.setPosition(
//                topLeft.x + path[i].second * cellW + cellW / 2.0f - c.getRadius().x / 2,
//                topLeft.y + path[i].first * cellH + cellH / 2.0f - c.getRadius().y / 2);
//        window.draw(c);
